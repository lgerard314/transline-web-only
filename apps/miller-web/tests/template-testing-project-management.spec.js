...
// Parity gate for the project-management sandbox (components-v2) vs the real page.
// PM has no video embeds; only .mw-rel__track (RelatedServices carousel scroll offset)
// is non-deterministic and must be masked. .mw-lyt is included harmlessly (absent from PM).
...
const REAL = "http://localhost:3001/industrial-services/project-management";
const TT = "http://localhost:3001/template-testing/project-management";
...
async function sectionClasses(page) {
  return page.$$eval(
    ".mw-svc-hero, .mw-why, .mw-pm, .mw-svc-cta, .mw-svc-related-sec",
...
async function sectionBoxes(page) {
  return page.$$eval(
    ".mw-svc-hero, .mw-why, .mw-pm, .mw-svc-cta, .mw-svc-related-sec, footer",
...
test("PM: section set + order parity", async ({ page }) => {
...
test("PM: desktop page-body byte parity (hero-down, excludes header chrome)", async ({ page }) => {
