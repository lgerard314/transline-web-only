...
// Parity gate for the industrial-cleaning sandbox (components-v2) vs the real page.
// IC has no video embeds, so only .mw-rel__track (RelatedServices carousel scroll offset)
// is non-deterministic and must be masked. .mw-lyt is included in the mask array as a
// harmless no-op (absent from IC). Desktop visual comparison clips from .mw-svc-hero
// downward to exclude the global-header nav active-underline chrome artifact.
const MASK_SELECTORS = [".mw-lyt", ".mw-rel__track"];
function masks(page) { return MASK_SELECTORS.map((s) => page.locator(s)); }

const REAL = "http://localhost:3001/industrial-services/industrial-cleaning";
const TT = "http://localhost:3001/template-testing/industrial-cleaning";
...
async function sectionClasses(page) {
  return page.$$eval(
    ".mw-svc-hero, .mw-svc-inds, .mw-fleet, .mw-why, .mw-svc-cta, .mw-svc-related-sec",
...
async function sectionBoxes(page) {
  return page.$$eval(
    ".mw-svc-hero, .mw-svc-inds, .mw-fleet, .mw-why, .mw-svc-cta, .mw-svc-related-sec, footer",
...
test("IC: section set + order parity", async ({ page }) => {
...
test("IC: desktop page-body byte parity (hero-down, excludes header chrome)", async ({ page }) => {
