...
/* Auto-scroll zoom collage — a mosaic of crew photos that scales up, "diving" into
   ...
   THIS SECTION PINS ON EVERY SURFACE, portrait included (logan 2026-06-11): its
   choreography converges to a full-screen centred focal point — "wait until the
   section fills the screen, then zoom into the screen" — and the pinned hold IS that
   mechanism, so un-pinning it breaks the design rather than adapting it. (The
   portrait-never-pins house rule targets pins that hold OVERSIZED STATIC compositions
   with dead space — who-we-serve / lifetime / VBEC — not this.) The only thing that
   varies by surface is the MOSAIC LAYOUT: portrait/small/short screens (COMPACT
   surfaces, the PIN_MQS complement, mirrored by the media block in home/careers.css)
   re-compose the spread in stage-relative % with a larger-resting centre tile.
...
// COMPACT surfaces — portrait / small / short screens. Same pin + dive mechanism; only
// the mosaic layout differs: the centre tile rests larger (1/2.4 ≈ 42% of the stage,
// vs 25% on the wide spread) so the portrait collage reads as photos, not slivers; the
// off-centre tiles are laid out AT their rest size (cap 1 — they sweep off early, so
// layout-size crispness headroom isn't needed and compositor layers stay small).
const COMPACT_ZOOM = [2.4, 5, 6, 5, 6, 8, 9];
const COMPACT_CAP = [2.4, 1, 1, 1, 1, 1, 1];
// Wide-spread gate — MUST mirror the compact media block in home/careers.css (which is
// this list's exact complement). Selects the mosaic layout ONLY — the pin runs on
// every surface.
...
export function ZoomCollage01({ photos = [], children, intro, autoScroll = true, diveInitialSlope = 1 }) {
  ...
  useEffect(() => {
    ...
    // Wide spread vs compact mosaic — mirrors the CSS layout gate exactly; refreshed
    // on resize/rotation. Layout only; the pin/dive mechanism is identical in both.
    let wideMode = pinMqs.every((m) => m.matches);
    const syncMode = () => { wideMode = pinMqs.every((m) => m.matches); };
    let raf = 0;
    ...
    const apply = (pd, raw = pd) => {
      const zoomArr = wideMode ? ZOOM : COMPACT_ZOOM;
      const capArr = wideMode ? CAP : COMPACT_CAP;
    ...
    const loop = (ts) => {
      ...
      // The sticky stage holds (EVERY surface — the pinned hold is this choreography's
      // mechanism) while the dive plays over diveDist of track; the section rests
      // through its whole entry (raw 0 until the stage reaches the viewport top).
    ...
    // the matchMedia changes directly so the layout mode can never go stale.
