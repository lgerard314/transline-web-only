...
- **CAPTURE RECIPE (learned from the pilot):** on this page the sticky pin tracks (facility/careers/sectors/lifetime) make `scrollIntoView`/`getBoundingClientRect` scroll-targeting unreliable for lower sections — element/viewport screenshots can come back blank. The pins are **gated off under `prefers-reduced-motion: reduce`**, so launch the browser context with `reducedMotion: 'reduce'` to collapse the page to normal flow; then `scrollIntoViewIfNeeded()` + `element.screenshot()` of your section works cleanly. Use reduced-motion for the FONT verification. Then do a SEPARATE pass with motion ON (default) to verify the PIN behavior itself.
