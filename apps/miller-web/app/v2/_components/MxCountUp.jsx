...
// React re-render), rAF easeOutCubic. The final value is rendered once as an
// invisible SIZER so the live numeral never reflows its frame. Animating digits
// are aria-hidden; an sr-only span carries the final value so screen readers
// announce it once. prefers-reduced-motion shows the final value immediately.
//
...
export function MxCountUp({ value, suffix = "", duration = 1600, play = true, onStart, onComplete, className }) {
  ...
  return (
    <span className={`mx-cu ${className || ""}`}>
      ...
      <span className="mx-cu__live" ref={liveRef} aria-hidden="true" />
