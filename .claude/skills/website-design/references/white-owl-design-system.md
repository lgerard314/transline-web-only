...
4. **Numbered "article" framing.** Items and cards carry zero-padded numbers (`01`, `02`, …) in mono; the hero carries a small mono caption (`mw-hero__article`). The whole page is composed to read like a numbered document/dossier, not a generic marketing scroll. (Scope note, logan 2026-06-12: numbers belong on COMPOSITIONAL devices — cards in a row, route stations, plates — NEVER as leading markers on vertical/stacked list rows; stacked lists use the house diamond mark instead. See the no-numbers rule in references/design-language.md.)
