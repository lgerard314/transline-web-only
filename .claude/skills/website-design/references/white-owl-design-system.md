<!-- MOVED 2026-06-12 from docs/DESIGN-SYSTEM.md — now the single source of truth inside the website-design skill. Section (§) numbers cited anywhere in the repo refer to THIS file. -->

**The user's explicit brief outranks every rule in this document.**
...
| Section title (`mw-section-title`) | Condensed | `clamp(32px, 3.4vw, 60px)` | 600, UPPERCASE | line-height 1.02, letter-spacing 0.005em, ends with a `.mw-stop` period glyph. |
...
1. **The clay "stamp" period.** A terracotta full stop on the text baseline after display headings — rendered as a literal `"."` GLYPH (`::before content: "."` in the Bahnschrift / "Arial Black" / `--font-condensed` stack, inheriting size/weight/baseline), NOT a drawn square box (the box was retired 2026-06-12; a glyph aligns, wraps, scales, and recolors like text). `.mw-stop` closes section titles; `.mw-hero__stop` is the same mark on the hero. It is the core brand mark. Reproduce by appending `<span className="mw-stop" aria-hidden="true" />` (or wrapping the title in `StopText01`); recolor with `color:`, never `background:`.
...
4. **Numbered "article" framing — NARROWED (logan 2026-06-12, second ruling).** Bare zero-padded sequential numerals (`01`, `02`, …) in the mono/eyebrow register are now banned as item markers in EVERY composition — cards in a row, equipment plates, route/station nodes, custody strips, and (per the first ruling) stacked lists. The house diamond mark takes the slot everywhere (hollow→filled doubles as a state cue). What survives of the dossier motif: worded document captions (the hero's `mw-hero__article` line) and designed display numerals that are part of a composition's typography rather than an eyebrow-style marker. See the forbidden-list entry in references/vocabulary.md.
...
7. **The unified photographic grade (SANCTIONED EXCEPTION — do not extend).** This pre-existing global token is the ONE allowed piece of photo color management: it stays as shipped on published layers, must not be strengthened/duplicated/added ad hoc, and is no precedent for color-managing photos in prompts or new CSS — photos are otherwise prompted and rendered NEUTRAL per references/imagery.md (logan, 2026-06-12). Every photograph wears one warm "LUT" — a single `--mw-photo-grade` filter token (`saturate(0.9) contrast(1.05) sepia(0.1) brightness(1.01)`) that calms over-saturated shots, adds a little contrast to flat ones, and pulls cool/neutral images toward the clay palette, so the imagery reads as one art-directed set instead of mixed stock. It is applied **only to photographic layers** (hero, bento tiles, sector cards, facility gallery, careers bleed) and **never** to logos, the blue ISO seals, affiliate marks, social icons, or the truck/vector graphics — those keep their true colour (§2). It is non-destructive (a CSS `filter`, no asset edits); tune the whole page from the one token in `04-home.css`.
...
These are the visual primitives the design is built from. On miller-web they are now packaged as the **`components-v2` template library** — you compose pages from `06_sections` templates rather than hand-assembling these classes (see `apps/miller-web/references/white-owl-templates.md (this skill)` and the live catalog at `/template-gallery`). The class vocabulary below still describes what those templates render and is the reference when building or extending a template. Class names are the Miller (`mw-*`) versions used on the home page.
...
On miller-web you assemble this rhythm by **composing `06_sections` templates** in a thin `page.jsx`, each fed a `content` object (see `references/white-owl-templates.md (this skill)`). To add or convert a section so it slots in: reach for an existing `06_sections` template first (`/template-gallery`); pick a surface consistent with its neighbors (a long light run is fine — dark anchors are deliberate punctuation); the template already handles `.mw-inner`, the eyebrow → title (+ stop) → lead opening, and scroll-reveal (§9). Templates may carry backwards-compatible variants — e.g. `HoverCardGrid01` drops its stat cycle and collapses to a single-column header when no `stats` are supplied. Only when no template fits do you extend one (a backwards-compatible config knob) or add a new `*-02` template — never hand-build a bespoke section into the route. Two adjacent dark sections or a jarring surface jump break the cadence — avoid them.
...
Do:
- **Compose from the `components-v2` templates first** (miller-web): build the page as a thin `page.jsx` ordering `06_sections` templates fed by `content` (see `references/white-owl-templates.md (this skill)`; browse `/template-gallery`). Extend a template only via a backwards-compatible config knob, or add a new `*-02` template when the DOM diverges — and run `npm run template-map` + Playwright-verify affected pages before/after any template change. The visual rules below are what those templates already embody.
- **Run the page as a one-session production job per `references/white-owl-responsive-playbook.md (this skill)` §7**: light motion budget chosen up front from the proven vocabulary, templates trusted (verify seams + content edge cases + the page's own motion, not template internals), ONE parallel verification pass at the end.
- **Design contextually**: ask who lands on this page and what they need before choosing the treatment — this is a hazardous-waste operator's working site, not a startup splash page; utilitarian pages get utilitarian-excellent design (unique, on-language, never a generic grand-marketing layer over a booking flow).
- **Generate imagery with codex** (playbook §7.5): ultra-realistic purpose-fit images per design instead of reusing the library; transparent-bg needs → request the page's bg color; truck shots → attach `miller-env-*` fleet reference photos so generated trucks match the real fleet.
...
- **Related services is one shared rail everywhere.** Every service page closes with the `RelatedServices` client component (`.mw-rel`) — a horizontal scroll-snap track of uniform landscape 4:3 cards with squared prev/next controls and an "All services" link. **The cards carry the HOME SERVICES picture-card treatment (logan 2026-06-12):** full-bleed photo IN COLOR at rest (no grayscale on the rail), light top wash, the roster bottom content scrim, cqw-sized condensed title (#d9d9d9 → #fff), the action arrow pinned top-right (→ turns amber ↗ on hover), and the two-line summary in a 0fr→1fr hover/focus reveal — the rail card rules live self-sufficiently under `.mw-rel__item` in `05-service-shared.css` (the legacy `.mw-svcs-tile` base styles ship only with the dev galleries). It lists every service except the current page; do not hand-roll a per-page related-services grid.
