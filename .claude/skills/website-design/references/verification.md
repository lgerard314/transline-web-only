...
- Non-desktop captures use REAL device dimensions — width AND height (390×844, 430×932, 768×1024, …) with `isMobile`/`hasTouch`/`deviceScaleFactor` — never a width-only window with an arbitrary height; cut-offs are judged against the true device viewport (full list + rationale in responsive.md). Desktop reference frame: 1440×900.
