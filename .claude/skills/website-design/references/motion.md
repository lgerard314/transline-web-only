...
**Scroll-reveal coverage floor — HARD RULE (logan, 2026-06-12; hardened 2026-06-13): NO section EVER ships with zero motion, and the MAJORITY of items must carry an on-appear reveal.** Larger containers that hold smaller items get scroll/scrub effects; the individual components inside get their own per-item appear-reveals; lists ALWAYS get per-item reveals; most text blocks too. Pressure valve: when one column/side of a section carries an effect, the other side may rest. Sections whose items already arrive via a scrub slice are covered; the failure mode is a block of meta/body text (or a list) that just sits there while its neighbors move. Interior pages get this breadth of SMALL/MEDIUM motion — the big pinned choreography stays home-only.

**Perceptibility caveat — "technically revealing" can still read as DEAD (logan, 2026-06-13).** The house `data-reveal` / `MillerScrollReveal` fires each element a quick 320ms `mw-rv` blip (12px settle) the instant its TOP crosses the viewport BOTTOM edge (rootMargin 0) — i.e. in the user's PERIPHERY, already settled by the time the item reaches where they actually look; and on a single-column list every child gets `--reveal-order 0`, so there's NO stagger either. Net effect: a list animates by the probe yet looks motionless on screen (logan caught CWC's "who we collect from" this way — interaction hover/swap does NOT count as appear-motion). Fix for lists and any items that must read as alive: port the home services-roster row reveal — each row rises out of an `overflow:hidden` `<li>` mask (translateY ~110% → 0, ~0.9s hero easing) triggered when the row is FULLY on screen via a bespoke per-row scroll check (reversible top-down cascade), so the motion plays centrally where the eye is. Do NOT "fix" this by widening the global observer's rootMargin (it's tuned to 0 for the home page on purpose). Worked refs: CWC §5 stream-index rows (mask rise) and §6 schedule steps (fade+rise). See memory [[motion-on-every-section-hard-rule]].
