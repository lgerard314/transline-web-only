...
**Hover motion is a SEPARATE category from scroll motion** — and unlike scroll moments (which vary per section), hover behavior should be CONSISTENT per element type across the site (cards lift the same way everywhere, links arrow the same way, photo frames zoom the same way, titles indent the same way). Design hover systems per element class, not per section — and don't underrate the micro-motions (a title indenting on hover, an arrow nudging, a diamond filling): those small touches are a large part of what makes a page feel alive, and their site-wide consistency is part of the home-foundation grammar (design-language.md).
