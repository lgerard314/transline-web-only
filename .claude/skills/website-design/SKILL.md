...
- **Approved sections carry reference screenshots** ([approved/INDEX.md](references/approved/INDEX.md)) — consult them before building similar work; any change that alters an approved section's rendered appearance MUST refresh its shots + index rows in the same commit — via the MECHANICAL capture harness (`scripts/capture-home.mjs`, background process), never agent-walked capture.
