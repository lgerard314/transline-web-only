// process-flow.jsx — "How we do it" custom animated flow
// Two exports: <ProcessFlowSimple> (Home) and <ProcessFlowAdvanced> (Process page)
// Both use the same 6 steps + shared icon set.
...
const PROC_STEPS = [
  {
    ...
    longBlurb: "Project intake. The customer reaches out with material details, volume, location, and timeline — by phone, the intake form, or referral. We confirm scope and timing.",
  ...
  },
  {
    ...
    longBlurb: "We review the waste profile, analytical data, and SDS — and confirm a Canadian receiving facility will accept it. If data is missing, we coordinate sampling.",
  ...
  },
  {
    ...
    longBlurb: "For hazardous material, we coordinate the cross-border permit workflow with U.S. EPA and Environment & Climate Change Canada — including notices, contracts, insurance, and movement documents.",
  ...
  },
  {
    ...
    longBlurb: "We schedule the movement: authorized carrier, route, pickup and border crossing windows, documentation packets, and on-the-ground contacts at both ends.",
  ...
  },
  {
    ...
    longBlurb: "Material is picked up, accompanied by the movement document, crosses the border at the authorized port of entry, and is delivered to Miller Environmental Corporation in Canada.",
  ...
  },
  {
    ...
    longBlurb: "Material is processed: recycled or repurposed where viable, otherwise treated and disposed at the authorized facility. We return confirmation documentation for your records.",
...
function StepIcon({ name, size = 44 }) {
  ...
  switch (name) {
    case "inquiry":
      return (
        <svg {...base}>
          <path d="M8 12 a3 3 0 0 1 3 -3 h26 a3 3 0 0 1 3 3 v16 a3 3 0 0 1 -3 3 h-18 l-6 6 v-6 h-2 a3 3 0 0 1 -3 -3 z"
                fill={c} />
          <text x="24" y="26" textAnchor="middle" fontSize="18" fontWeight="700"
                fill="#fff" fontFamily="var(--font-display)">?</text>
        </svg>
      );
    case "profile":
      // 3 test tubes
      return (
        <svg {...base}>
          <g transform="translate(10 7)">
            {[0, 9, 18].map((x, i) => (
              <g key={i}>
                <path d={`M${x} 2 v22 a4 4 0 0 0 8 0 v-22 z`}
                      stroke={c} strokeWidth="2" fill="none" />
                <path d={`M${x + 0.6} ${10 + i * 2} v${14 - i * 2} a3.4 3.4 0 0 0 6.8 0 v-${12 - i * 2} z`}
                      fill={c} />
      ...
      return (
        <svg {...base}>
          ...
          <g>
            ...
            {/* dotted connector */}
            <path d="M16 12 q 8 6 16 0" stroke={c} strokeWidth="1.5"
                  strokeDasharray="1.5 2" fill="none" />
          </g>
          {/* truck */}
      ...
      return (
        <svg {...base}>
          ...
          <path d="M24 14
                   ...
                   l3.4 0.6
                   z"
                fill={c} stroke="#fff" strokeWidth="0.6" strokeLinejoin="round" />
        </svg>
      );
    case "recycle":
      return (
...
// ─── ProcessFlowSimple — for the Home page ──────────────────────────────────
// 6 step nodes laid out horizontally with a connecting accent line and arrows.
// Uses scroll-reveal for entry.
function ProcessFlowSimple({ onNav }) {
  return (
    <div className="tlpf">
        ...
        {PROC_STEPS.map((s, i) => (
          <div className="tlpf__step" key={s.key}>
            ...
            {i < PROC_STEPS.length - 1 && (
              <span className="tlpf__arrow" aria-hidden="true">→</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="tlpf__footer" data-reveal>
        <span className="tl-mono" style={{ color: "var(--c-ink-3)" }}>
          See the full process →
        </span>
        <button className="tl-btn tl-btn--dark" onClick={() => onNav && onNav("process")}>
...
// ─── ProcessFlowAdvanced — for the Cross-Border Process page ────────────────
// Zigzag layout: 6 rows, each row containing its own stem segment so the
// vertical line is ALWAYS exactly as tall as the row's card. No measurement,
// no SVG, no transitions on height — just simple per-row composition.
function ProcessFlowAdvanced() {
  const containerRef = useRef(null);
  const rowRefs = useRef([]);
  const [activeStep, setActiveStep] = useState(-1);

  // Scroll tracking — highest visible step
  useEffect(() => {
    ...
    while (scrollEl) {
      ...
      if ((cs.overflowY === "auto" || cs.overflowY === "scroll")
          && scrollEl.scrollHeight > scrollEl.clientHeight) break;
    ...
    <div ref={containerRef} className="tlpfx">
      <div className="tlpfx__head" data-reveal>
        ...
        <p className="tl-lead" style={{ marginTop: 16, maxWidth: "60ch" }}>
          Six steps, drawn out in full. Each step has its own deliverable, its
          own parties, and its own typical timing. Scroll to advance the truck.
        </p>
      </div>
      ...
      <div className="tlpfx__stage">
        {PROC_STEPS.map((s, i) => {
          ...
          return (
            ...
            >
              {/* The stem column — always present in the center */}
              <div className="tlpfx__stem">
                {/* Top half — dashed bg, with optional accent overlay */}
                <div className="tlpfx__stem-half tlpfx__stem-half--top"
                     data-active={active && !isFirst ? "1" : "0"}
                     data-hidden={isFirst ? "1" : "0"} />
                {/* Node dot at the center */}
                <div className="tlpfx__stem-node" data-active={active ? "1" : "0"} />
                {/* Bottom half — dashed bg, accent if NEXT step is also active */}
                <div className="tlpfx__stem-half tlpfx__stem-half--bot"
                     data-active={i < activeStep ? "1" : "0"}
                     data-hidden={isLast ? "1" : "0"} />

                {/* Truck — only rendered in the active row, slides smoothly */}
                {isTruck && activeStep >= 0 && (
                  <div className="tlpfx__truck" data-active={activeStep >= 0 ? "1" : "0"}>
                    <svg width="40" height="40" viewBox="0 0 40 40">
                      <circle cx="20" cy="20" r="18" fill="var(--c-bg)"
              ...
              {/* Branch + card on the correct side */}
...
Object.assign(window, { ProcessFlowSimple, ProcessFlowAdvanced, StepIcon, PROC_STEPS });
