...
const ContactPage = ({ onNav, tweaks }) => {
  const [step, setStep] = __useState(0);
  const [form, setForm] = __useState(initialForm);
  const [errors, setErrors] = __useState({});
  const [submitted, setSubmitted] = __useState(false);
    ...
    return (
      <div className="tl-pagewrap">
        <PageHero
          ...
          title={<>Thanks. We've got your project<span style={{ color: "var(--c-accent)" }}>.</span></>}
          ...
          ctas={<>
            <button className="tl-btn tl-btn--primary" onClick={() => onNav("home")}>
              Back to home <span className="tl-btn-arr">→</span>
            </button>
            <button className="tl-btn tl-btn--ghost-light" onClick={() => { setSubmitted(false); setStep(0); setForm(initialForm); }}>
  ...
  return (
    <div className="tl-pagewrap">
      <PageHero
        ...
        title={<>Start a cross-border waste <span style={{ color: "var(--c-accent)" }}>project.</span></>}
            ...
            {[
              {
                tag: "PHONE",
                t: "(314) 934-2133",
                b: "Reach the office direct.",
                cta: "Call now",
                href: "tel:+13149342133",
                icon: "phone",
              },
              {
                tag: "OFFICE",
              ...
              {
                tag: "PROJECT INTAKE",
                t: "Submit material details",
                b: "A team member will review the inquiry.",
                cta: "Begin form ↓",
                href: "#tl-intake",
                icon: "doc",
              },
              ...
              <div key={i} style={{
                ...
                background: i === 2 ? "var(--c-navy)" : "var(--c-surface)",
                color: i === 2 ? "var(--c-on-navy)" : "var(--c-ink)",
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <span style={{
                    width: 36, height: 36, borderRadius: 999,
                    border: `1px solid ${i === 2 ? "rgba(255,255,255,0.16)" : "var(--c-line)"}`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    color: i === 2 ? "#fff" : "var(--c-blue)",
                  }}>
                    <Icon name={c.icon} />
                  </span>
                ...
                <h3 style={{
                  ...
                  letterSpacing: "-0.015em", margin: 0, lineHeight: 1.15,
                  color: i === 2 ? "#fff" : "var(--c-navy)",
                }}>
                  {c.t}
                </h3>
                <p style={{ flex: 1,
                            color: i === 2 ? "rgba(230,238,242,0.78)" : "var(--c-ink-2)",
      ...
      <section className="tl-section" id="tl-intake" style={{ paddingTop: 0 }}>
        <div className="tl-container" data-tlgrid-collapse style={{ display: "grid", gridTemplateColumns: "0.7fr 1.3fr", gap: 56, alignItems: "start" }}>
          <div data-tlsticky style={{ position: "sticky", top: 96 }}>
            ...
            <div style={{ marginTop: 32, padding: 20, background: "var(--c-surface)",
                          border: "1px solid var(--c-line)", borderRadius: 10 }}>
              <div className="tl-mono" style={{ fontSize: 11, color: "var(--c-ink-3)", letterSpacing: "0.1em", marginBottom: 12 }}>
                PREFER TO TALK?
              </div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 22, color: "var(--c-navy)", marginBottom: 4, fontWeight: 500 }}>
                (314) 934-2133
              </div>
          ...
          <div className="tl-form-shell">
            ...
            <div className="tl-form-progress">
              {WIZARD_STEPS.map((s, i) => (
                <div key={s.key} className="tl-form-progress__step"
                     data-active={i === step ? "1" : "0"}
                     data-done={i < step ? "1" : "0"}
                     onClick={() => { if (i < step) setStep(i); }}>
                  <div className="num">{s.num}</div>
                  <div className="lbl" dangerouslySetInnerHTML={{ __html: s.label }} />
                </div>
              ))}
            </div>
            ...
            <div className="tl-form-body">
              {step === 0 && (
                <div>
                  <h3 className="tl-display tl-display--s" style={{ marginBottom: 24 }}>
                    Who are we talking to?
                  </h3>
                  <div className="tl-field-row">
                    <FormField label="Name" required value={form.name} err={errors.name}
                               onChange={(v) => update("name", v)} placeholder="Jane Operator" />
                    <FormField label="Company" required value={form.company} err={errors.company}
                               onChange={(v) => update("company", v)} placeholder="Acme Materials" />
                  </div>
                  <div className="tl-field-row">
                    <FormField label="Email" required type="email" value={form.email} err={errors.email}
                               onChange={(v) => update("email", v)} placeholder="jane@acme.com" />
                    <FormField label="Phone" value={form.phone} type="tel"
                               onChange={(v) => update("phone", v)} placeholder="(555) 123-4567" />
                  </div>
                  <FormField label="Company location" value={form.companyLocation}
                             onChange={(v) => update("companyLocation", v)} placeholder="City, State" />
                </div>
              )}
              ...
              {step === 1 && (
                <div>
                  ...
                  <FormField label="Waste description" required textarea
                             value={form.description} err={errors.description}
                             onChange={(v) => update("description", v)}
                             placeholder="Describe the material in plain language — what it is, what process it came from, anything notable about it." />

                  <div className="tl-field">
                    <label>Classification <span className="req">*</span></label>
                    <div className="tl-pillset">
                      {["Hazardous", "Non-hazardous", "Unknown / TBD"].map((c) => (
                        <button type="button" key={c} className="tl-pill"
                                data-on={form.classification === c ? "1" : "0"}
                  ...
                  <div className="tl-field-row" style={{ gridTemplateColumns: "2fr 1fr" }}>
                    <FormField label="Estimated volume" value={form.volume}
                               onChange={(v) => update("volume", v)} placeholder="e.g. 150" />
                    <div className="tl-field">
                      <label>Unit</label>
                  ...
                  <div className="tl-field">
                    ...
                    <div className="tl-pillset">
                      <button type="button" className="tl-pill" data-on={form.hasData ? "1" : "0"}
                              onClick={() => update("hasData", true)}>Yes — I can share</button>
                      <button type="button" className="tl-pill" data-on={!form.hasData ? "1" : "0"}
                              onClick={() => update("hasData", false)}>Not yet</button>
                    </div>
                  </div>
              ...
              {step === 2 && (
                <div>
                  <h3 className="tl-display tl-display--s" style={{ marginBottom: 24 }}>
                    Where is it and when does it need to move?
                  </h3>
                  <FormField label="Material location" required value={form.materialLocation} err={errors.materialLocation}
                             onChange={(v) => update("materialLocation", v)} placeholder="Address or facility, City, State" />
                  ...
                  <div className="tl-field">
                    ...
                    <div className="tl-pillset">
                      {[
                        ...
                        <button type="button" key={t} className="tl-pill"
                                data-on={form.timeline === t ? "1" : "0"}
              ...
              {step === 3 && (
                <div>
                  ...
                  <div className="tl-field">
                    ...
                    <div className="tl-pillset">
                      {[
                        ...
                        <button type="button" key={s} className="tl-pill"
                                data-on={form.scopeNeeded.includes(s) ? "1" : "0"}
                  ...
                  <FormField label="Anything else we should know?" textarea
                             value={form.message}
                             onChange={(v) => update("message", v)}
                             placeholder="Current disposal challenge, prior shipping history, special handling, deadlines, etc." />
                </div>
              )}

              {step === 4 && (
                <div>
                  <h3 className="tl-display tl-display--s" style={{ marginBottom: 8 }}>
                    Review &amp; send.
                  </h3>
              ...
              <div style={{
                ...
                marginTop: 32, paddingTop: 24, borderTop: "1px solid var(--c-line)",
              }}>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  {step > 0 && (
                    <button className="tl-btn tl-btn--ghost" onClick={goBack}>
                      ← Back
                    </button>
                  )}
      ...
      <section className="tl-section" style={{ background: "var(--c-surface)", borderTop: "1px solid var(--c-line)" }}>
        <div className="tl-container" data-tlgrid-collapse style={{ display: "grid", gridTemplateColumns: "0.7fr 1fr", gap: 64, alignItems: "start" }}>
          ...
          <div>
            <FAQ items={[
              { q: "Do you help with hazardous waste permitting?",
                a: "Yes. TransLine49° assists with cross-border hazardous waste permitting." },
              { q: "Do you coordinate logistics?",
                a: "Yes. TransLine49° assists with scheduling the logistical movement of material." },
              { q: "Do you handle non-hazardous waste too?",
                a: "Public company language references both hazardous and non-hazardous waste disposal needs." },
            ]} />
          </div>
        </div>
      </section>
  ...
  return (
    <div style={{ border: "1px solid var(--c-line)", borderRadius: 10, padding: "20px 22px", marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <span className="tl-mono" style={{ fontSize: 11, letterSpacing: "0.1em", color: "var(--c-blue)" }}>
          {label}
        </span>
        <button onClick={onEdit} style={{ background: "transparent", border: 0, cursor: "pointer",
                                            fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.08em",
                                            color: "var(--c-ink-3)", textTransform: "uppercase" }}>
          EDIT
        </button>
      </div>
