...
- **`good-integration`:** feed the agent an ordered run of real approved home shots (consecutive `references/approved/home/desktop/` section shots) framed as "consecutive APPROVED sections — tonal transitions between sections are intentional; judge only genuine cross-boundary breaks" → must verdict PASS / no blocker. (Approved shots are coherent by construction; the framing prevents false-flagging the intended dark→light transitions the spec-audit identified — the settled run is DARK→CREAM→DARK by design, `06-vbec-settled` settles cream.)
- **`broken-integration`:** a single deterministically-generated frame (Playwright generator) that recolors a cream-field section to **on-palette dark walnut (`#3B2418`)** — a dark slab INTERRUPTING the page's continuous cream field. A CONTINUITY/seam defect, NOT a palette violation (the color is on-palette), so it exercises cross-boundary judgment rather than a cheap palette catch → must verdict ISSUES + a blocker citing continuity/seam. Deterministic (recolor a KNOWN section — `.mw-roster2`'s ancestor), NOT a luminance auto-find (the spec-audit showed the dark LifetimeReel→VBEC field doesn't exist as a static computed background).
- **Discrimination gate = the rubber-stamp regression:** the existing always-PASS stub must make `broken-integration` FAIL (reuse `run.mjs`'s `EVAL_SYSPROMPT` seam + case loop). This — NOT a fragile "current-agent-without-the-scope" RED (which a flagrant defect would defeat) — is what proves the integration judge isn't vacuous, matching how the original eval's discrimination is proven.
