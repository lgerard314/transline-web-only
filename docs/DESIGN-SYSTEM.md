...
6. **The chain-of-custody thread.** A 1px vertical clay line at the left margin that reinforces the "documented chain from dock to disposal" story. It *starts* in the hero as a static clay gradient hairline (`.mw-hero::after`, at `clamp(20px, 3vw, 60px)`) and *continues* on the home page as a fixed full-height **custody thread** (`.mw-thread`, driven by `MillerCustodyThread`) pinned to the same x: a faint base line whose clay fill "draws" downward as the page is scrolled, with a small diamond stamp travelling at its leading edge — the further you read, the more of the line is "on record." Decorative (`aria-hidden`), **home only**, hidden below 1200px where the gutter can't clear the content, and under reduced motion it renders as a static full-height clay rule with no travelling stamp (see §9).
...
**The custody thread (home).** The fixed left-margin chain-of-custody thread (motif §5.6) draws its clay fill and travels its diamond stamp in lock-step with scroll progress — `MillerCustodyThread` writes `--mw-thread-p` (0→1) once per frame (rAF, passive) and all visual work is CSS (`scaleY` + a `top` offset).
...
**Reduced motion is mandatory.** Every animation has a `@media (prefers-reduced-motion: reduce)` branch that disables it: reveals snap to visible, cycles freeze on their first item, the count-up renders its final value immediately, the underline-draws appear instantly, the hero entrance (zoom, kinetic type, fades) and the scroll parallax are switched off (the hero renders settled), and the custody thread drops its travelling stamp to render as a static full-height clay rule. Any new animation **must** ship its reduced-motion off-switch in the same change. (Verified: under `prefers-reduced-motion: reduce` the hero line-in, parallax wrap, and thread fill all compute to no transform / `scaleY(1)`.)
