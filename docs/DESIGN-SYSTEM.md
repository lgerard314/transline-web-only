...
- **Run the page as a one-session production job per `docs/RESPONSIVE-PLAYBOOK.md` §7**: light motion budget chosen up front from the proven vocabulary, templates trusted (verify seams + content edge cases + the page's own motion, not template internals), ONE parallel verification pass at the end.
