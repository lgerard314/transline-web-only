...
**Before designing any responsive translation, read `docs/RESPONSIVE-PLAYBOOK.md` in full** — it carries the mandatory pre-edit process (desktop-intent inventory → per-surface translation plan → written motion contracts), the approved design vocabulary (rails/strips with peek, directional edge bleeds, white media cards, stagger rows, portrait type scale), and the worked implementation recipes (pin gates, flow writers + var slices, `display:contents` re-composition, reversed scrollers, sticky-hover handling). The bullets below are the verification half; the playbook is the design half.
...
- **Full-screen / pinned (scroll-jacked) sections are a LANDSCAPE-only signature.** Gate every pin on `(orientation: landscape)` + a sensible min-height + `prefers-reduced-motion: no-preference`, and mirror the gate in the section's JS (`canPin`-style) so JS state classes can't fire without the CSS pin. **Portrait never pins — any width, any pointer.** Non-pinned (flowing) surfaces need explicit content insets: a panel edge and the content start often coincide by construction, gluing headers to the container edge. Only the home hero is ever full-viewport-height on portrait.
